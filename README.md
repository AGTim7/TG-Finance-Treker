...
- история транзакций с пагинацией, фильтрами типа и периода;
...
Главная, история и аналитика подключены к production API. Страницы полезных материалов и настроек пока являются заготовками.
...
| `GET` | `/api/transactions?page=1&limit=20&type=EXPENSE&from=...&to=...` | Возвращает фильтрованные транзакции и сводку с пагинацией |
| `GET` | `/api/analytics?type=EXPENSE&from=...&to=...` | Возвращает аналитику по категориям за период |
| `GET` | `/api/analytics/overview` | Возвращает доходы, расходы и баланс последних шести месяцев |
