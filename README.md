...
- кошельки с общим балансом, стандартными счетами «Банковский» и «Наличные» и пользовательскими счетами;
- история транзакций с пагинацией, фильтрами типа, периода и кошелька;
- аналитика категорий с кольцевой диаграммой и общая динамика доходов/расходов за шесть месяцев;
- общие и пользовательские категории доходов и расходов;
- редактирование и удаление транзакций из главной страницы и истории;
- архивирование пользовательских категорий и кошельков без потери истории;
- экспорт до 10 000 транзакций в оформленный XLSX с выбором периода и отправкой файла в чат;
...
Главная, история, аналитика, полезные инструменты и настройки подключены к API.
...
- ExcelJS и PNGJS для финансовых XLSX-отчётов и встроенных диаграмм;
- Vercel Functions;
...
| `GET` | `/api/categories?type=EXPENSE` | Возвращает глобальные и пользовательские категории |
| `POST` | `/api/categories` | Создает пользовательскую категорию |
| `PATCH` | `/api/categories/:id` | Изменяет пользовательскую категорию |
| `DELETE` | `/api/categories/:id` | Архивирует пользовательскую категорию |
| `GET` | `/api/wallets` | Возвращает кошельки, их балансы и общий баланс |
| `POST` | `/api/wallets` | Создает пользовательский кошелёк |
| `PATCH` | `/api/wallets/:id` | Изменяет кошелёк или назначает его основным |
| `DELETE` | `/api/wallets/:id` | Архивирует неосновной кошелёк с нулевым балансом |
| `GET` | `/api/dashboard?walletId=...` | Возвращает данные главной страницы с фильтром кошелька |
| `POST` | `/api/transactions` | Создает транзакцию пользователя |
| `PATCH` | `/api/transactions/:id` | Изменяет транзакцию пользователя |
| `DELETE` | `/api/transactions/:id` | Удаляет транзакцию пользователя |
| `GET` | `/api/transactions?page=1&limit=20&type=EXPENSE&walletId=...&from=...&to=...` | Возвращает фильтрованные транзакции и сводку с пагинацией |
| `GET` | `/api/analytics?type=EXPENSE&walletId=...&from=...&to=...` | Возвращает аналитику по категориям за период |
| `GET` | `/api/analytics/overview?walletId=...` | Возвращает доходы, расходы и баланс последних шести месяцев |
| `POST` | `/api/exports/transactions` | Формирует XLSX за период и отправляет его пользователю через Telegram-бота |
...
Ограничения: сумма должна быть положительной, содержать не более двух знаков после запятой и не превышать `9999999999.99`; описание может содержать до 500 символов; `limit` пагинации не может превышать 50. Один экспорт содержит не более 10 000 транзакций; endpoint экспорта допускает до пяти запросов за десять минут на пользователя. XLSX потоково формируется во временном каталоге ОС, передаётся в Telegram как файловый поток и удаляется в блоке `finally`; постоянная копия отчёта не хранится.
...
| `wallet` | Кошельки, начальные остатки и признак основного кошелька |
| `transaction` | Доходы и расходы с сохраненным типом, кошельком и категорией |
...
Backend-тесты проверяют Telegram-аутентификацию, валидацию запросов и структуру XLSX-отчёта. Frontend-тесты проверяют форматирование данных и границы периода экспорта. Перед production-деплоем необходимо запускать весь набор команд.
...
- добавить фильтр истории по конкретной категории;
- добавить переход из месячного столбца к операциям выбранного месяца;
- добавить редактирование и удаление транзакций;
